...
if [[ "$imagebuildtime" > "$sptime" ]] && [[ "$imagebuildtime" > "$smtime" ]] && [[ "$imagebuildtime" > "$custtime" ]] && [ $(gcloud container images describe gcr.io/$gcp_project_id/$name:latest > /dev/null ; echo $?) == 0 ];
then
  echo "gcloud image"
  ...
  echo "build starting"
  (cd build/$fn_name && gcloud builds submit --tag gcr.io/$gcp_project_id/$fn_name:latest --verbosity=debug --timeout "1h" >$project_dir/builderror && touch $project_dir/buildsuccess) || echo "build failed"
  echo "build done"
  time=$(date +%s)
