...
declare -a aptpackages=("default-jre" "openssh-server" "tmux" "autossh" "zip" "unzip" "curl" #APT_PACKAGES#);
if [ "$COLAB" != "True" ]
then
...
then
    ...
    mkdir -p #PROJECTDIR#serve && ln -s #PROJECTDIR#serve /
    cd #PROJECTDIR#serve
...
#SHOWCASEFLASKSETUP#
if [ "$SHOWCASEFLASKSETUP" == "True" ]
then
    FLASKPATH="#PROJECTDIR#showcase/flask_app.py"
    if [ -f $FLASKPATH ];
    then
        echo "Flask file present"
    else
        send_status "Setting up Showcase flask"
        mkdir -p #PROJECTDIR#showcase && ln -s #PROJECTDIR#showcase /
        cd #PROJECTDIR#showcase
        wget -q -NS --content-disposition https://clouderizer-serving-projects.s3.us-east-2.amazonaws.com/flask_app.py &> /dev/null
        sed -i "s/#SECRET_KEY#/#SHOWCASE_SECRET_KEY#/1" ./flask_app.py
        sed -i "s/#SHOWCASE_BASE_URL#/#SBASE_URL#/1" ./flask_app.py
    fi
fi
...
if [ "$H2OSETUP" == "True" ]
then
    send_status "Setting up H2O v3 Web Flow"
    H2ODIR="#PROJECTDIR#data/h2o/"
    if [[ ! -d $H2ODIR ]]
    then
        mkdir -p $H2ODIR
        cd $H2ODIR
        wget http://h2o-release.s3.amazonaws.com/h2o/rel-yu/2/h2o-3.28.0.2.zip
        unzip -qq h2o-3.28.0.2.zip
    fi
fi
...
if [ "$DAISETUP" == "True" ]
then
    send_status "Setting up H2O Driverless AI"
    DAIDIR="#PROJECTDIR#data/dai/"
    if [[ ! -d $DAIDIR ]]
    then
        mkdir -p $DAIDIR
        cd $DAIDIR
        wget https://s3.amazonaws.com/artifacts.h2o.ai/releases/ai/h2o/dai/rel-1.8.1-146/x86_64-centos7/dai-1.8.1.1-linux-x86_64.sh
        chmod 755 dai-1.8.1.1-linux-x86_64.sh 
        ./dai-1.8.1.1-linux-x86_64.sh
        cd dai-1.8.1.1-linux-x86_64
        ./run_dai.sh
    fi
fi
...
echo 'PATH=$PATH:/usr/local/cuda-9.0/bin' >> /root/.profile
...
cd $HOME_DIR
BASE_URL=$BASE_URL CLDZ_HOME=$HOME_DIR CLDZ_LOCALE=$CLDZ_LOCALE CLDZ_PLAT=DOCKER java -jar clouderizer.jar #PROJECT_KEY# &> /tmp/cldz.out
#check for WSL
